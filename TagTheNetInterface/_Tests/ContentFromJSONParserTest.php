...
class ContentFromJSONParserTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testWithNullJSON() {
        ...
        $this->assertEquals("testId", $contentItem->GetId());
    }
    ...
    public function testWithFullGoodJSON() {
        ...
        $this->assertEquals(13, count($item->GetTags()));
        $tags = $item->GetTags();
        $firsttag = $tags[0];
        $this->assertEquals("what", $firsttag->GetType());
        $this->assertEquals("knallgrau", $firsttag->GetText());
        $lasttag = $tags[12];
        $this->assertEquals("who", $lasttag->GetType());
        $this->assertEquals("Dieter Rappold", $lasttag->GetText());
    }
